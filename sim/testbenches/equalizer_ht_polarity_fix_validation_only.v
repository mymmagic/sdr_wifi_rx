...
// 802.11 pilot polarity sequence for HT DATA. Bit 0 is the first HT DATA
// symbol after L-SIG and two HT-SIG symbols have consumed the first 3 entries.
...
wire csi_capture_window = ((!ht && num_ofdm_sym == 1) || (ht && !ht_csi_done));
assign csi_valid = (csi_capture_window && state == S_CPE_ESTIMATE && sample_in_strobe_dly == 1 && enable && (~reset));
wire csi_data_sbc_valid = ((csi_valid == 1) && ((HT_DATA_SUBCARRIER_MASK[lts_raddr-1] && ht==1)||(DATA_SUBCARRIER_MASK[lts_raddr-1] && ht==0))) ? 1 : 0;
            ...
            S_GET_POLARITY: begin
                ...
                if (ht) begin
                    current_polarity <= {
                        ...
                        ht_polarity[3], // 21
                        ht_polarity[2]  // 7
                        } ^ {4{~HT_PILOT_POLARITY[ht_polarity_idx]}};
                    ht_polarity <= {ht_polarity[0], ht_polarity[3:1]};
                    ht_polarity_idx <= (ht_polarity_idx == 126 ? 0 : ht_polarity_idx + 1);
                end else begin
                    current_polarity <= {
                        polarity[0],    // -7
                        polarity[0],    // -21
