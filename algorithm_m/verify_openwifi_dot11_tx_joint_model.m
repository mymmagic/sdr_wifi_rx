function verify_openwifi_dot11_tx_joint_model(case_name, rate_nibble, psdu_len_bytes)
%VERIFY_OPENWIFI_DOT11_TX_JOINT_MODEL Generate vectors for dot11_tx joint sim.

if nargin < 1 || isempty(case_name)
    case_name = 'legacy_6M_len8';
end
if nargin < 2 || isempty(rate_nibble)
...
payload_word = case_payload_word(rate_nibble, psdu_len_bytes);
joint = openwifi_dot11_tx_joint_model(rate_nibble, psdu_len_bytes, payload_word);
...
fprintf(fid, 'openwifi dot11_tx module-chain joint reverse model\n');
fprintf(fid, 'Case: %s\n', case_name);
fprintf(fid, 'Source RTL top: C:\\wifi\\openwifi-hw-master\\openwifi-hw-master\\ip\\openofdm_tx\\src\\dot11_tx.v\n');
fprintf(fid, 'Scenario: legacy %s, rate nibble=0x%X, LENGTH=%d bytes, payload source word=0x%016s\n', ...
    joint.rate_name, joint.rate_nibble, joint.psdu_len_bytes, dec2hex(joint.payload_word, 16));
...
fprintf(fid, 'Joint chain covered: L-SIG/DATA bit source, DATA scrambler/FCS, convolutional encoder,\n');
fprintf(fid, 'puncture/interleaver, ram_simo packing, modulation, pilot/DC/sideband mapping.\n');
...
fprintf('Wrote dot11_tx joint vectors to: %s\n', out_dir);
end

function payload_word = case_payload_word(rate_nibble, psdu_len_bytes)
